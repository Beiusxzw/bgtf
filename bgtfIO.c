...
static int BGTFSerializeRecord(void *dest, int *dlen, BGTFRecord_t *record, int *slen)
{
    ...
    memcpy(dest + p + 8, &(record->start), 8);
    p += 16;
...
uint8_t BGTFWriteHdr(BGTF *file)
{
    ...
    file->hdr->summary_offset = ftell(__stream__); // record the summary offset at 64 bytes
    if (fwrite(p, sizeof(uint8_t), 32, __stream__) != 32)
        return 10;                            // SumaryOffset + 32 bytes empty

    file->hdr->cl_offset = ftell(__stream__); // record the chromosome list offset at SumaryOffset + 40 bytes empty
    if (BGTFWriteChromList(__stream__, file))
        return 11;
    if (writeAtPos(&(file->hdr->cl_offset), sizeof(uint64_t), 1, 0x8, __stream__))
        return 7;
    ...
    if (writeAtPos(&(file->hdr->record_offset), sizeof(uint64_t), 1, 0x10, __stream__))
        return 8;
    free(p);
    return 0;
}
