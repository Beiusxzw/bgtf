...
// RTreeSetAllocator allows for configuring a custom allocator for
// all rtree library operations. This function, if needed, should be called
// only once at startup and a prior to calling RTreeInit().
...
static void rect_expand(uint64_t *rect, uint64_t *other, int dims) {
...
static int rect_largest_axis(uint64_t *rect, int dims) {
    int axis = 0;
    uint64_t size = rect[dims+0]-rect[0];
    for (int i = 1; i < dims; i++) {
        uint64_t nsize = rect[dims+i]-rect[i];
        if (nsize > size) {
            axis = i;
            size = nsize;
        }
                               ...
                               uint64_t *rect)
{
    ...
    for (int i = 0; i < node->count; i++) {
        ...
        uint64_t min_dist = crect[axis] - axis_min;
        uint64_t max_dist = axis_max - crect[dims+axis];
...
static uint64_t rect_enlarged_area_d(uint64_t *rect, uint64_t *other, int dims, 
                                   uint64_t *area_out)
{
    uint64_t enlarged = MAX(other[dims+0], rect[dims+0]) - MIN(other[0], rect[0]);
    uint64_t area = rect[dims+0] - rect[0];
    for (int i = 1; i < dims; i++) {
...
static uint64_t rect_enlarged_area_2(uint64_t *rect, uint64_t *other, int dims, 
                                   uint64_t *area_out)
{
    uint64_t area = (rect[dims+0] - rect[0]) *
                  (rect[dims+1] - rect[1]);
...
static uint64_t rect_enlarged_area_3(uint64_t *rect, uint64_t *other, int dims, 
                                   uint64_t *area_out)
{
    uint64_t enlarged = 
        (MAX(other[dims+0], rect[dims+0]) - MIN(other[0], rect[0])) *
...
static uint64_t rect_enlarged_area_4(uint64_t *rect, uint64_t *other, int dims, 
                                   uint64_t *area_out)
{
    uint64_t enlarged = 
        (MAX(other[dims+0], rect[dims+0]) - MIN(other[0], rect[0])) *
...
static int \
fn_subtree(uint64_t *rects, int nrects, uint64_t *rect, int dims) { \
    uint64_t *crect = rects; \
    int j = 0; \
    uint64_t j_area; \
    uint64_t j_enlargement = fn_rect_enlarged_area(crect, rect, dims, &j_area); \
    for (int i = 1; i < nrects; i++) { \
        crect += dims*2; \
        uint64_t area; \
        uint64_t enlargement = fn_rect_enlarged_area(crect, rect, dims, &area); \
...
static void attempt_reinsert(struct RTree *rtree) {
    ...
    while (rtree->reinsert) {
        ...
        while (node->count > 0) {
            ...
            if (!RTreeInsert_x(rtree, rect, item)) {
...
// RTreeInsert inserts an item into the rtree. This operation performs a copy
// of the data that is pointed to in the second and third arguments. The R-tree
// expects a rectangle, which is an array of uint64_ts, that has the first N
...
static uint8_t inter_d(uint64_t *rect, uint64_t *other, int dims) {
...
static uint8_t inter_2(uint64_t *rect, uint64_t *other, int dims) {
    return !(rect[dims+0] < other[0] || rect[0] > other[dims+0] ||
             rect[dims+1] < other[1] || rect[1] > other[dims+1]);
}
static uint8_t inter_3(uint64_t *rect, uint64_t *other, int dims) {
    return !(rect[dims+0] < other[0] || rect[0] > other[dims+0] ||
             rect[dims+1] < other[1] || rect[1] > other[dims+1] ||
             rect[dims+2] < other[2] || rect[2] > other[dims+2]);
}
static uint8_t inter_4(uint64_t *rect, uint64_t *other, int dims) {
    return !(rect[dims+0] < other[0] || rect[0] > other[dims+0] ||
             rect[dims+1] < other[1] || rect[1] > other[dims+1] ||
             rect[dims+2] < other[2] || rect[2] > other[dims+2] ||
...
static const uint64_t svg_scale = 20.0;
...
{
    uint64_t *min = rect;
    uint64_t *max = &rect[rtree->dims];
    uint8_t point = min[0] == max[0] && min[1] == max[1];
    if (node) {
        if (!node->leaf) {
            for (int i = 0; i < node->count; i++) {
    ...
    if (point) {
        uint64_t w = (max[0]-min[0]+1/svg_scale)*svg_scale*10;
        fprintf(f, 
                            ...
                            uint64_t *rect, int height, int depth)
{
    assert(node->count);
    assert(height);
    uint64_t rect2[8*rtree->dims*2]; // VLA
    for (int i = 0; i < node->count; i++) {
        uint64_t *child_rect = rect_at(rtree, node, i);
...
static uint8_t rand_alloc_fail = 0;
...
static void copy_rand_point(int dims, uint64_t *point) {
    for (int i = 0; i < dims; i++) {
        if (i == 0) {
            point[i] = ((uint64_t)rand()/(uint64_t)RAND_MAX) * 360.0 - 180.0;
        } else if (i == 1) {
            point[i] = ((uint64_t)rand()/(uint64_t)RAND_MAX) * 180.0 - 90.0;
        } else {
            point[i] = ((uint64_t)rand()/(uint64_t)RAND_MAX) * 100;
        }
        point[dims+i] = point[i];
    }
}

static uint8_t search_iter(const uint64_t *rect, const void *item, void *udata) {
    (*(int*)udata)++;
    return 1;
}
...
static uint8_t tsearch(struct RTree *rtree, int dims, uint64_t *rect, int index) {
    ...
    RTreeSearch(rtree, rect, single_iter, &ctx);
    return ctx.found;
}

static void test(int N, int dims) {
    uint64_t *rects;
    uint8_t use_cities = 0;
#ifdef CITIES
    if (N ==0 && dims == 0) {
        use_cities = 1;
        ...
        assert(rects);
        for (int i = 0; i < N; i++) {
            uint64_t *point = &rects[dims*2*i];
            point[0] = (uint64_t)cities[i*2+0] / 1000.0 - 180.0;
            point[1] = (uint64_t)cities[i*2+1] / 1000.0 - 90.0;
            point[2] = (uint64_t)cities[i*2+0] / 1000.0 - 180.0;
            point[3] = (uint64_t)cities[i*2+1] / 1000.0 - 90.0;
        }
    }
#endif
    if (!use_cities) {
        ...
        assert(rects);
        for (int i = 0; i < N; i++) {
            uint64_t *rect = &rects[dims*2*i];
            for (int j = 0; j < dims; j++) {
                rect[j] = (uint64_t)rand()/RAND_MAX * 100;
                rect[dims+j] = rect[j] + (uint64_t)rand()/RAND_MAX;
            }
        }
    }
    int *vals;
    ...
    for (int i = 0; i < N; i++) {
        int index = vals[i];
        uint64_t *rect = &rects[dims*2*index];
        assert(!RTreeDelete(rtree, rect, &index));
        assert(!tsearch(rtree, dims, rect, index));
        while (!RTreeInsert(rtree, rect, &index)){}
        assert(RTreeCount(rtree) == i+1);
        assert(tsearch(rtree, dims, rect, index));
        assert(RTreeDelete(rtree, rect, &index));
        assert(RTreeCount(rtree) == i);
        assert(!tsearch(rtree, dims, rect, index));
        assert(!RTreeDelete(rtree, rect, &index));
        while(!RTreeInsert(rtree, rect, &index)){}
        assert(RTreeCount(rtree) == i+1);
    ...
    for (int i = 0; i < N; i++) {
        ...
        uint64_t *rect = &rects[dims*2*index];
        assert(tsearch(rtree, dims, rect, index));
        assert(RTreeCount(rtree) == N-i);
        RTreeDelete(rtree, rect, &index);
        assert(!tsearch(rtree, dims, rect, index));
        assert(RTreeCount(rtree) == N-i-1);
        rtree_deep_check(rtree);
    }
    rtree_deep_check(rtree);
...
#define bench(name, N, code) {{ \
    ...
    clock_t end = clock(); \
    uint64_t elapsed_secs = (uint64_t)(end - begin) / CLOCKS_PER_SEC; \
    uint64_t bytes_sec = (uint64_t)bytes/elapsed_secs; \
    printf("%d ops in %.3f secs, %.0f ns/op, %.0f op/sec", \
        N, elapsed_secs, \
        elapsed_secs/(uint64_t)N*1e9, \
        (uint64_t)N/elapsed_secs \
    ); \
    if (bytes > 0) { \
    ...
    if (total_mem > tmem) { \
        ...
        printf(", %.2f bytes/op", (uint64_t)used_mem/N); \
    } \
    if (total_allocs > tallocs) { \
        size_t used_allocs = total_allocs-tallocs; \
        printf(", %.2f allocs/op", (uint64_t)used_allocs/N); \
    } \
    printf("\n"); \
}}
...
static void benchmarks() {
    ...
    uint64_t *coords = xmalloc(8*dims*2*N);
    for (int i = 0; i < N; i++) {
    ...
    bench("search", N, {
        uint64_t *point = &coords[dims*2*i];
        int res = 0;
        RTreeSearch(rtree, point, search_iter, &res);
        assert(res == 1);
    });

    bench("replace", N, {
        uint64_t *point = &coords[dims*2*i];
        RTreeDelete(rtree, point, &(int){i});
        RTreeInsert(rtree, point, &(int){i});
    });
...
int main() {
    RTreeSetAllocator(xmalloc, xfree);
    if (getenv("BENCH")) {
